...
| 🐝 **Plein Essaim**     | L'autonomie RÉELLE : la ruche décide (`deciderPas`), agit, et se cadence toute seule. **Deux interrupteurs en série** — le niveau, choisi par l'utilisateur, et `HIVE_RUNNER=off\|on`, choisi par l'hôte qui paie le temps-machine (défaut : `off`). Un cycle par projet et par minute, recul exponentiel puis pause après 5 échecs, et le gros bouton rouge arrête AVANT l'effet. Elle **délibère** (conseil), **planifie** (un verdict devient une tâche) et **corrige** (une erreur vue sur plusieurs machines vient du code) ; livrer et fusionner restent à brancher, et le Journal le DIT. `essaim_cycle` au Journal. |

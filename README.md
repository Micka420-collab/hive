...
| 🐝 **Plein Essaim**     | L'autonomie RÉELLE : la ruche décide (`deciderPas`), agit, et se cadence toute seule. **Deux interrupteurs en série** — le niveau, choisi par l'utilisateur, et `HIVE_RUNNER=off\|on`, choisi par l'hôte qui paie le temps-machine (défaut : `off`). Un cycle par projet et par minute, recul exponentiel puis pause après 5 échecs, et le gros bouton rouge arrête AVANT l'effet. `essaim_cycle` au Journal. |

...
function evs(type: string, count: number, ts: number): Array<{ type: string; ts: number }> {
...
describe('lireTemperature (module pur)', () => {
  ...
  it('pondérations : 1 échec + 1 retry + 1 refus + 1 succès → (1 + 0.6 + 0.8) × 25 = 60°', () => {
    const lecture = lireTemperature(
      [
        { type: 'task_failed', ts: now },
        { type: 'task_retry', ts: now },
        { type: 'task_rejected', ts: now },
...
describe('Thermorégulation : câblage scheduler', () => {
  ...
  it('surchauffe confirmée sur deux ticks : thermo_shift émis et concurrence réduite à 2/4', () => {
    ...
    expect(shift?.payload).toMatchObject({ bande: 'surchauffe', temperature: 84, facteur: 0.5 });
    expect(String(shift?.payload.message)).toContain('Thermorégulation');

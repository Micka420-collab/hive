...
## A. Le code tient — ✅ mesuré (arbre `6379854` + ce commit, 23 août 2026)
...
| Suite de bancs           | `npm test` (vitest run)                                  | ✅ **4894** (4886 verts, 8 ignorés, **0 rouge**)                                                                                                                                                                            |
...
| Rien de neuf n'est nu    | `npm run loupe` (mutation sur le diff ajouté)            | ✅ terrain de la note de l'horloge (base `6379854`) : **17 examinées, 3 nues → 3 fermées, contre-rejeu 3/3** — l'affichage l'avait précédé (base `1b93c44`, 18 examinées, 2 nues → 2 fermées) (§ terrain, `docs/ETAPES.md`) |

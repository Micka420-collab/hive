...
## A. Le code tient — ✅ mesuré en CI (arbre `f019d5ca11dd3ce19d588b71e32dc731d1ba8a7d`, 25 septembre 2026, `ubuntu-latest` / Node 24)
...
>
> **LA RÉPARTITION CI-DESSOUS EST DÉRIVÉE, ET ÇA SE DIT.** Le total (5498) est
> mesuré ici. Les 5490 verts ne le sont pas : ils viennent des **5476 verts
> confirmés en CI** sur `574e596` (run 33312462566), plus les **quatorze** bancs
> de `liens-des-documents` — qui ne sont conditionnés ni à une plateforme ni à
> une version de Node, donc s'exécutent partout.
>
> C'est une inférence sur deux quantités mesurées, pas un chiffre écrit de tête,
> et elle est écrite ici pour qu'on puisse la refuser. **Le procédé a été éprouvé
> deux fois :** les deux dérivations précédentes (5466 + 6 = 5472, puis
> 5472 + 4 = 5476) ont été posées de la même façon et CONFIRMÉES par la jambe qui
> juge. Ça ne les rend pas vraies d'avance — si celle-ci est fausse,
> `compte-tests.mjs` rougit sur `ubuntu-latest` au prochain tour, et c'est très
> bien ainsi.
>
...
| Suite de bancs           | `npm test` (vitest run)                                  | ✅ **6076** (6062 verts, 14 ignorés, **0 rouge**)                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Trois OS × Node 24       | matrice CI `ubuntu` / `windows` / `macos`                | ⏳ en cours (run à publier après CI de cette PR ; matrice de tests passée, garde des compteurs restampée dans cette branche)                                                                                                                                                                                                                                                                                                                                      |

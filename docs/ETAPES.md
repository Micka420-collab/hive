...
| 12  | **Le Cerveau** — le savoir qui survit à la fenêtre de contexte       | 🟡   | **Les deux moitiés livrées.** Pur (`src/shared/cerveau.ts`) : genres, consolidation à 3 récurrences, budget de contexte avec REFUS sur invariant, élagage par usage, santé du graphe. Impur (`src/cerveau-reel.ts`) : lecture/écriture du dossier de notes, confinement éprouvé, liens symboliques refusés, élagage réel. Inscrit dans `empreinte.ts` sous « cerveau », non retirable — l'ajout a fait rougir **trois** gardes, donc il a été conscient. 45 tests, la loupe ne voit rien de nu. **Reste 🟡 : AUCUNE OUVRIÈRE NE LE LIT ENCORE.** Il manque le branchement sur le prompt via `pourLaTache()`. Un module complet et débranché reste un module débranché. |

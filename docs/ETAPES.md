...
| 13  | **La contre-expertise** — faire relire une IA par une AUTRE          | 🟡   | **Module PUR livré et testé** (`src/shared/contre-expertise.ts`, 16 tests) : le choix des relecteurs refuse tout modèle IDENTIQUE au producteur — les angles morts d'un modèle sont par construction ce qu'il ne voit pas — et REFUSE plutôt que de dégrader quand aucun autre modèle n'est en ligne. Un relecteur par modèle, choix total, `shell` simulé écarté, production passée en bloc de données (injection de modèle à modèle), une seule objection suffit à contester. **Reste 🟡 : RIEN NE L'APPELLE.** Il manque la dispatch d'une tâche de relecture vers le nœud choisi et la remontée de l'avis. Le module DÉCIDE ; personne ne lui demande encore.                                                                                                        |

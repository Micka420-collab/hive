...
| 12  | **Le Cerveau** — le savoir qui survit à la fenêtre de contexte       | 🟡   | **Module PUR livré et testé** (`src/shared/cerveau.ts`, 26 tests) : genres, consolidation à 3 récurrences, budget de contexte avec REFUS sur invariant, élagage par usage, santé du graphe. **Reste 🟡 et surtout pas ✅ : rien ne l'appelle encore.** Il manque la moitié impure (lire/écrire le dossier), l'inscription à `empreinte.ts`, et le branchement sur le prompt des ouvrières. Un module que rien n'appelle est exactement ce que ce fichier existe pour ne pas laisser cocher. |

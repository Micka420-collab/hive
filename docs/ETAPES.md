...
| 12  | **Le Cerveau** — le savoir qui survit à la fenêtre de contexte       | 🟡   | **Lu, injecté, borné — et vérifié de bout en bout.** `tests/cerveau-wiring.test.ts` monte une vraie ruche, branche un nœud en WebSocket et constate qu'un invariant posé à la main dans le dossier arrive dans le `assign_task`, encadré comme une DONNÉE. Le mutant qui retire le savoir du contexte rougit deux tests. Le Cerveau passe AVANT la Couveuse (les invariants ne se font pas déloger par une leçon d'échec) et se sert en premier sur le budget ; s'ils ne tiennent pas, l'événement `cerveau_refus` l'annonce plutôt que de laisser une ouvrière partir sans eux en silence. 50 tests. **Reste 🟡, et le manque est précis : LA BOUCLE D'AMÉLIORATION N'EST PAS FERMÉE.** Rien n'écrit d'épisode automatiquement, et `aConsolider` — la promotion d'un épisode répété trois fois en règle — n'a aucun appelant. La ruche LIT son cerveau ; elle ne l'ÉCRIT pas encore. |
